...
def preprocess_images(images, preprocess_function, target_size=(256, 256)):
    """
    ...
    # Process each resized image using the provided preprocessing function
    processed_images = [
        preprocess_function(img) for img in tqdm(resized_images, desc="Processing images")
    ]
...
def get_sobel_features_updated(image, threshold=0.5):
    """
    Compute Sobel edge detection on an image. Binarize the edge intensity to white
    ...
    edges = sobel(image_gray)
    #return edges
    # thresholding to binarize the edge intensity image
